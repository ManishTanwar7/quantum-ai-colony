...
export default function ProbabilityChart({ probabilities = {}, counts = {}, shots = 1024 }) {
  ...
  return (
    <div className="flex flex-col bg-quantum-surface border border-quantum-border rounded-2xl p-4 shadow-xl">
                ...
                <div className="h-3 w-full bg-slate-900 rounded-full overflow-hidden p-0.5 border border-slate-800">
                  <motion.div
                    ...
                    className={`h-full rounded-full ${
                      pct > 0 
                        ? 'bg-gradient-to-r from-cyan-500 via-teal-400 to-purple-500 shadow-[0_0_8px_rgba(0,240,255,0.4)]'
                        : 'bg-transparent'
                    }`}
                  />
                </div>
      ...
      <div className="mt-3 pt-3 border-t border-quantum-border/60 text-[10px] text-slate-400 flex items-center justify-between font-mono">
